...
L.BrowserPrint.Utils = {
    ...
    initialize: function (customRenderers) {
        ...
        this.registerLayer(L.TileLayer, "L.TileLayer", function (layer, utils) {
            return L.tileLayer(layer._url, utils.cloneOptions(layer.options));
        });
        this.registerLayer(L.GridLayer, "L.GridLayer", function (layer, utils) {
            return L.gridLayer(utils.cloneOptions(layer.options));
        });
        this.registerLayer(
            L.ImageOverlay,
            "L.ImageOverlay",
        ...
        this.registerLayer(L.Marker, "L.Marker", function (layer, utils) {
            return L.marker(
                layer.getLatLng(),
                utils.cloneOptions(layer.options)
            );
        });
        this.registerLayer(L.Popup, "L.Popup", function (layer, utils) {
            return L.popup(utils.cloneOptions(layer.options))
                .setLatLng(layer.getLatLng())
                .setContent(layer.getContent());
        });
        this.registerLayer(L.Circle, "L.Circle", function (layer, utils) {
            return L.circle(
                layer.getLatLng(),
                layer.getRadius(),
                utils.cloneOptions(layer.options)
        ...
        this.registerLayer(L.Rectangle, "L.Rectangle", function (layer, utils) {
            return L.rectangle(
                layer.getBounds(),
                utils.cloneOptions(layer.options)
            );
        });
        this.registerLayer(L.Polygon, "L.Polygon", function (layer, utils) {
            return L.polygon(
                layer.getLatLngs(),
                utils.cloneOptions(layer.options)
            );
        ...
        this.registerLayer(L.GeoJSON, "L.GeoJSON", function (layer, utils) {
            return L.geoJson(
                layer.toGeoJSON(),
                utils.cloneOptions(layer.options)
            );
    ...
    cloneLayer: function (layer) {
        ...
        var factoryObject;
        if (layer._group) {
            // Exceptional check for L.MarkerClusterGroup
        ...
        if (!renderer) {
            for (var i = 0; i < this._cloneRendererArray.length; i++) {
                ...
                if (oldRenderer instanceof factoryObject.type) {
                    this._knownRenderers[oldRenderer._leaflet_id] =
                        factoryObject.builder(oldRenderer.options);
                    break;
                }
            }
