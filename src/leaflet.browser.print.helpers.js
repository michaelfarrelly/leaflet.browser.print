...
L.BrowserPrint.Helper = {
    ...
    getPaperSize: function (mode) {
        if (mode.options.pageSeries) {
            ...
            if (
                mode.options.pageSeriesSize &&
                mode.options.pageSeriesSize !== "0"
            ) {
    ...
    getSize: function (mode, orientaion = "Portrait") {
        ...
        var topbottom =
            orientaion === "Portrait"
                ? parseFloat(pageMargin.top) + parseFloat(pageMargin.bottom)
                : parseFloat(pageMargin.left) + parseFloat(pageMargin.right);
        var leftright =
            orientaion === "Portrait"
                ? parseFloat(pageMargin.left) + parseFloat(pageMargin.right)
                : parseFloat(pageMargin.top) + parseFloat(pageMargin.bottom);
