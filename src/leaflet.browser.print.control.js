...
L.Control.BrowserPrint = L.Control.extend({
        ...
        for (var i = 0; i < this.options.printModes.length; i++) {
			...
			*/
            if (mode.length) {
                ...
                mode = L.BrowserPrint.Mode[mode](
                    "A4",
                    this._getDefaultTitle(key)
                );
            ...
            if (this.options.printModes.length === 1) {
                ...
                clickContainer = L.DomUtil.create(
                    "ul",
                    "browser-print-holder",
                    container
                );
                mode.Element = L.DomUtil.create(
                    "li",
                    "browser-print-mode",
                    clickContainer
                );
            ...
            L.DomEvent.on(
                clickContainer,
                "click",
                mode.options.action(this.browserPrint, mode),
                this.browserPrint
            );
    ...
    _getDefaultTitle: function (key) {
        return (
            (this.options.printModesNames &&
                this.options.printModesNames[key]) ||
            key
        );
    },
...
L.control.browserPrint = function (options, browserPrint) {
    ...
    if (
        options &&
        options.printModes &&
        (!options.printModes.filter || !options.printModes.length)
    ) {
