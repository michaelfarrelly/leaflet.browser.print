...
L.BrowserPrint = L.Class.extend({
    ...
    _print: function (printMode, autoBounds) {
        ...
        var overlay = this._addPrintMapOverlay(
            printMode,
            pageOrientation,
            origins
        );
        ...
        if (!this.options.debug) {
            this.printInterval = setInterval(function () {
                if (self.cancelNextPrinting || !self._map.isPrinting) {
                    ...
                    self._map.isPrinting &&
                    !self._isTilesLoading(overlay.map)
                ) {
                    clearInterval(self.printInterval);
                    if (self.options.manualMode) {
                        self._setupManualPrintButton(
                            overlay.map,
                            origins,
                            overlay.objects
                        );
                    } else {
                        self._completePrinting(
                            overlay.map,
                            origins,
                            overlay.objects
                        );
    ...
    _addPrintMapOverlay: function (printMode, pageOrientation, origins) {
        ...
        var printSize = L.BrowserPrint.Helper.getSize(
            printMode,
            pageOrientation
        );
